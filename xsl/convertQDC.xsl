...
<xsl:for-each select="/*/*">
	...
	</td><td><a href="{$baseUrl}/search/digital_repository/dc.{$FIELD}:{$DATA}"><img title="Find Similar by {$FIELD}" src="{$PATH}/images/view.gif" alt="Find Similar by {$FIELD}" /></a></td></tr>
	</xsl:if>
	</xsl:for-each>
